...
\end{align*}
where $u(t,\vec{x})$ denotes the oil density at position $\vec{x}$ at time $t$.
...
Altough it is not used the simulation supports a parameter for time $t$ to account for any development in the ocean currents.
...
Let $u_i^n$ denote the oil density in triangle cell $i$ with area $A_i$, at time $t^n$ .
The scaled outward normal $\vec{\nu}_{i,\ell}$ associated with edge $\ell$ of
cell $i$, pointing into its neighbour cell $\text{ngh}_\ell$.
The velocity field is denoted by $\vec{v}(\vec{x},t)$. The scaled normals are fixed and precomputated
for every cell edge, and stored in a dictionary related to 
...
Execute the following command in a terminal to run one specific config file:

\begin{verbatim}
python main.py -f configs .\configs\"Specific config file"
\end{verbatim}

Execute the following command in a terminal to run multiple config files:

\begin{verbatim}
python main.py -f configs --find_all
\end{verbatim}

Running \textbf{\texttt{main.py}} will use \textbf{\texttt{input.toml}} as configuration file.
...
\chapter{Results}
In this section, we'll cover how the entire process resulted
in a highly scalable, well structured and optimized package.
We'll also cover the results of actual simulations running with
different parameters, and interesting observations made.
\\\\
The program is very scalable. The process of adding a new cell type
...
dict in CellFactory. That's it, all other math and related data is generalized
for all convex polygons.
\\\\
The structure of the code has made it intuitive to use, where a full simulation
can be run with just a few lines. We have also worked hard to ensure that data
handling happens where we felt it made sense. Geometric data handling regarding individual
...
and easy to modify.
\\\\
We also worked hard to optimize the code, having a single simulation running in less than
...
\newpage
All in all, this led to the following results:
\begin{center}
  \begin{tabular}{| c | c | c | c |}
    \hline
    & Default & Longer & Simple \\
    \hline
    \hline
    nSteps & 500 & 2500 & 500 \\
    \hline
    tEnd & 0.5 & 2.5 & 0.5 \\
    \hline
    meshName & bay.msh & bay.msh & simple.msh \\
    \hline
    borders & [[0.0, 0.45], [0.0, 0.2]] & [[0.0, 0.45], [0.0, 0.2]] & [[0.0, 0.45], [0.0, 0.2]] \\
    \hline
    logName & log & log & log \\
    \hline
    writeFrequency & 20 & 10 & 20 \\
    \hline
    \hline
    Oil in fishing grounds at end & 19.33 && \\
    \hline
    Runtime on Macbook Air M4 &&& \\
    \hline
  \end{tabular}
\end{center}

There were some interesting observations in these. The first one is that
as tEnd gets long enough for the oil to reach the mesh borders, the visualization
messes up by making the oil suddenly "disappear". This is because the oil density
in certain cells starts exceeding the limit of the colorbar,
leading to a sudden change in the color scale. This was fixed by clamping the visualized
oil density values to the set limit of the colorbar.
