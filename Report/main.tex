...
The standard upwind flux from cell \(I\) to neighbor \(\text{ngh}\) is
\[
...
Since the velocity field is constant, we can precompute the outgoing flow
\[
\phi_{I \rightarrow \text{ngh}} = \max\!\big(0, \langle \vec{v}_{I,\text{ngh}}, \vec{\nu}_{I,\text{ngh}} \rangle \big)
\]
for every cell-neighbor pair. The oil leaving cell \(I\) over a time step \(\Delta t\)
is
\[
\Delta u_I = - \frac{\Delta t}{A_I} \sum_{\text{ngh}} \phi_{I \rightarrow \text{ngh}} \, u_I,
\]
while the neighboring cell receives a corresponding contribution scaled by its area
\[
\Delta u_{\text{ngh}} = \frac{\Delta t}{A_{\text{ngh}}} \, \phi_{I \rightarrow \text{ngh}} \, u_I.
\]
...
\[
u_I^{n+1} = u_I^n - \sum_{\text{ngh}} C_{I \rightarrow \text{ngh}} \, u_I^n,
\qquad
u_{\text{ngh}}^{\,n+1} = u_{\text{ngh}}^n + C_{\text{ngh} \leftarrow I} \, u_I^n,
\]
where \(C_{I \rightarrow \text{ngh}} = \frac{\Delta t}{A_I} \phi_{I \rightarrow \text{ngh}}\)
and \(C_{\text{ngh} \leftarrow I} = \frac{\Delta t}{A_{\text{ngh}}} \phi_{I \rightarrow \text{ngh}}\).

This formulation guarantees conservation of the total oil amount, since all outgoing flux
from a main cell is redistributed to its neighbors, scaled by the respective cell areas.
