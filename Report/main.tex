...
of Bay City is from the file  \texttt{bay.msh}. The data includes the coastline and ocean for the region.
...
The simulation supports parameter for time (t) in order to acoount for any change in the ocean currents,
and this reduce the effectiveness of precomputated values for the simulation. 

\section{Oil Exchange}
...
their update value, they add 

\section{Faucet Simulation}

When the velocity field is time-independent, its time derivative is zero:
\[
\frac{\partial \vec{v}}{\partial t} = \ [0, 0].
\]
As a consequence, the velocity at each cell remains constant for all times. For a triangle
cell \(I\) and one of its neighbors \(\text{ngh}\), the edge-averaged velocity is
\[
\vec{v}_{I,\text{ngh}} = \frac{1}{2}\Big(\vec{v}(\vec{x}_I) + \vec{v}(\vec{x}_{\text{ngh}})\Big),
\]
which is constant in time. Likewise, the scaled normal vector \(\vec{\nu}_{I,\text{ngh}}\) is
precomputed and does not change.

The standard flux from cell \(I\) to neighbor \(\text{ngh}\) is
\[
\text{Flux}_{I \rightarrow \text{ngh}} =
\begin{cases}
\langle \vec{v}_{I,\text{ngh}}, \vec{\nu}_{I,\text{ngh}} \rangle \, u_I, & \text{if } \langle \vec{v}_{I,\text{ngh}}, \vec{\nu}_{I,\text{ngh}} \rangle > 0,\\[2mm]
\langle \vec{v}_{I,\text{ngh}}, \vec{\nu}_{I,\text{ngh}} \rangle \, u_{\text{ngh}}, & \text{otherwise.}
\end{cases}
\]

However if we only consider the outgoing flux then
\[
\phi_{I \rightarrow \text{ngh}} = \langle \vec{v}_{I,\text{ngh}}, \vec{\nu}_{I,\text{ngh}} \rangle \cdot u_I
\]
for every cell-neighbor pair. The oil leaving cell \(I\) over a time step \(\Delta t\)
is
\[
\Delta u_I = 
- \frac{\Delta t}{A_I} \sum_{\text{ngh}} (\phi_{I \rightarrow \text{ngh}} \ \cdot u_I)
= u_I \cdot -\frac{\Delta t}{A_I} \sum_{\text{ngh}} \phi_{I \rightarrow \text{ngh}} \,
\]

while the neighboring cell receives a corresponding contribution scaled by its area
\[
\Delta u_{\text{ngh}} = u_I \cdot \frac{\Delta t}{A_{\text{ngh}}} \, \sum_{\text{ngh}} \phi_{I \rightarrow \text{ngh}} \
\]

Since u is the only time dependent variable we can factor out two constants from the formulas
\[
Flow_{I \rightarrow \text{ngh}} = \frac{\Delta t}{A_{\text{I}}} \, \sum_{\text{ngh}} \phi_{I \rightarrow \text{ngh}} 
\]
\[
Flow_{ngh \rightarrow \text{I}} = \frac{\Delta t}{A_{\text{ngh}}} \, \sum_{\text{ngh}} \phi_{I \rightarrow \text{ngh}} 
\]
By precomputing these coefficients for every cell-neighbor pair (the \emph{faucets}),
the simulation reduces to a simple lookup at each time step:
\[
u_I^{n+1} = u_I^n - Flow_{ngh \rightarrow \text{I}} \cdot  u_I^n,
\qquad
\]
\[
u_{\text{ngh}}^{\,n+1} = u_I^n + Flow_{ngh \rightarrow \text{I}} \cdot  u_I^n,
\]

This formulation guarantees conservation of the total oil amount, since all outgoing flux
from a main cell is redistributed to its neighbors and is scaled by their areas.
